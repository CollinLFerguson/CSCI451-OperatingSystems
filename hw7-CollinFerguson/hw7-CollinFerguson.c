...
{
	if(argc < 2){printf("You must supply a filename to the program\n"); exit(0);}
	...
	*pipeWriteEndFD = pipes[1]; //Converts the write end of the pipe into a string for passing as an arg to child
	...
	switch(pid)
	{
		case 0:
		{
			char *args[] = {"./childProcess", "/hw7-CollinFerguson", pipeWriteEndFD, argv[1], NULL}; //execvp requires an array of char*, the first element being the path.
			execvp(args[0], args);
